...
float getArea(float side1, float side2, float side3 ){
 ...
 void solution(){
  ...
  float abc[100];
  for(int i=0;i<100;i++){
    abc[i]=getArea(ab[i],bc[i],ac[i]);
  }
  float adc[100];
  for(int i=0;i<100;i++){
    adc[i]=getArea(ad[i],cd[i],ac[i]);
  }
  float abd[100];
  for(int i=0;i<100;i++){
    abd[i]=getArea(ab[i],ad[i],bd[i]);
  }
  float bcd[100];
  for(int i=0;i<100;i++){
    bcd[i]=getArea(bc[i],cd[i],bd[i]);
  }
